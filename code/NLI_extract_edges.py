...
class EdgeExtractor(BaseExtractior):
    ...
    @staticmethod
    def _get_messages_for_edge_direction(description: str, 
    ...
    def _extract_one_edge_gpt(self,
                             description: str, 
                             set_of_nodes: list[str],
                             pair_of_nodes: tuple[str, str], 
        ...
        arrow_type = self._get_completion_parsed_result(
            model=gpt_model,
            messages=self._get_messages_for_edge_direction(description, f"[{', '.join(set_of_nodes)}]", f"[{', '.join(pair_of_nodes)}]"),
    ...
    def extract_all_edges(self,
        ...
        for i, node_a in enumerate(set_of_nodes):
            for node_b in set_of_nodes[i+1:]:
                ...
                edge = self._extract_one_edge_gpt(description, set_of_nodes, (node_a, node_b), gpt_model, temperature)
    ...
    async def _async_extract_one_edge_gpt(self,
                                    description: str, 
                                    set_of_nodes: list[str],
                                    pair_of_nodes: tuple[str, str], 
                                    gpt_model: str = 'gpt-4o-mini',
        ...
        """
            Asynchronously extracts a directed edge between a pair of nodes using a specified GPT model.

            Args:
                description (str): The text description from which the edge needs to be extracted.
                set_of_nodes (List[str]): A list of node names to consider for edge extraction.
                pair_of_nodes (Tuple[str, str]): A tuple representing the pair of nodes to evaluate for an edge.
                gpt_model (str, optional): The GPT model to use for extraction. Defaults to 'gpt-4o-mini'.
                temperature (float, optional): The randomness of the model's output. Defaults to 0.0.

            Returns:
                Tuple[Optional[str], Optional[str]]: A tuple representing the directed edge or (None, None) if no edge is identified.
        """
        arrow_type = await self._async_get_completion_parsed_result(
            model=gpt_model,
            messages=self._get_messages_for_edge_direction(description, f"[{', '.join(set_of_nodes)}]", f"[{', '.join(pair_of_nodes)}]"),
            response_format=ArrowType,
            temperature=temperature,
    ...
    async def async_extract_all_edges(self,
                                description: str, 
                                set_of_nodes: list[str],
                                gpt_model: str = 'gpt-4o-mini',
                                temperature: float = 0,
        ...
        """
            Asynchronously extracts all possible directed edges between nodes in a set using a specified GPT model.

            Args:
                description (str): The text description from which edges need to be extracted.
                set_of_nodes (List[str]): A list of node names to consider for edge extraction.
                gpt_model (str, optional): The GPT model to use for extraction. Defaults to 'gpt-4o-mini'.
                temperature (float, optional): The randomness of the model's output. Defaults to 0.0.
                verbose (bool, optional): If True, prints each node pair being processed. Defaults to False.

            Returns:
                List[Tuple[Optional[str], Optional[str]]]: A list of tuples representing directed edges between nodes.
        """    
        edge_list = []
        ...
        for i, node_a in enumerate(set_of_nodes):
            for node_b in set_of_nodes[i+1:]:
                if verbose:
                    print(f"{node_a} # {node_b}")
                edge = await self._async_extract_one_edge_gpt(description, set_of_nodes, (node_a, node_b), gpt_model, temperature)
                if edge[0] is not None:
                    edge_list.append(edge)
