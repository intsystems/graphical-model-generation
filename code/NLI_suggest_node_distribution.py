...
class NodeDistributer(BaseExtractior):
    ...
    @staticmethod
    def _get_messages_for_vertex_distribution(description: str, node_name: str) -> tuple[dict, dict]:
        """
    ...
    def _suggest_vertex_distribution(self,
                                    description: str, 
                                    node_name: str, 
        ...
        assert distr_type in ['binary', 'categorical', 'continuous'], f"Wrong inferred distr type: {distr_type}"

        return distr_type
    

    def suggest_vertex_distributions(self,
                                    description: str, 
                                    node_names: list[str], 
                                    gpt_model: str = 'gpt-4o-mini', 
                                    temperature: float = 0
        ...
        for node_name in node_names:
            node_distr = self._suggest_vertex_distribution(description, node_name, gpt_model, temperature)
            node_distr_dict[node_name] = node_distr

        return node_distr_dict


    async def _async_suggest_vertex_distribution(self,
                                                description: str, 
                                                node_name: str, 
                                                gpt_model: str = 'gpt-4o-mini', 
                                                temperature: float = 0
        ...
        distr_type = await self._async_get_completion_result(
            model=gpt_model,
            messages=self._get_messages_for_vertex_distribution(description, node_name),
            temperature=temperature,
        )
    ...
    async def async_suggest_vertex_distributions(self,
                                                description: str, 
                                                node_names: list[str], 
                                                gpt_model: str = 'gpt-4o-mini', 
                                                temperature: float = 0
        ...
        node_distr_dict = {}
        for node_name in node_names:
            node_distr = await self._async_suggest_vertex_distribution(description, node_name, gpt_model, temperature)
            node_distr_dict[node_name] = node_distr
