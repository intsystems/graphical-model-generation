...
class TestNodeDistributer(unittest.TestCase):
    ...
    @mock.patch(
        "NLI_suggest_node_distribution.node_extraction_sys_message",
        "You are assistant.",
    )
    @mock.patch(
        "NLI_suggest_node_distribution.node_distribution_str_template",
        "Description: {description}, node_name: {node_name}",
    )
    def test_get_mess_text(self):

        format_dict = {"description": "my_description", "node_name": "node_name"}

        messages = self.node_distributer._get_messages_for_vertex_distribution(
            **format_dict
        )
        value = (
            {
                "role": "system",
                "content": NLI_suggest_node_distribution.node_extraction_sys_message,
            },
            {
                "role": "user",
                "content": NLI_suggest_node_distribution.node_distribution_str_template.format(
                    **format_dict
                ),
            },
        )
    ...
    @mock.patch(
        "NLI_suggest_node_distribution.NodeDistributer._get_completion_result",
    ...
    @mock.patch(
        "NLI_suggest_node_distribution.NodeDistributer._suggest_vertex_distribution",
        MagicMock(return_value="binary"),
    )
